...
# OpenCode reader bounds. The newest windows are kept and any truncation is
# reported explicitly so the UI can show coverage instead of silently hiding data.
MAX_OC_PART_PAGE = 2000
MAX_OC_PARTS_PER_SESSION = MAX_SESSION_USAGE_EVENTS
MAX_OC_PARTS_TOTAL = 250000
MAX_OC_MESSAGES_PER_SESSION = 800

# Soft wall-clock budget for a single OpenCode DB read. It sits below the
# read-only connection's hard 12s progress guard so a huge database yields a
# bounded, honestly-truncated partial snapshot instead of an interrupt that
# publishes nothing. The reader always processes at least the newest session.
OPENCODE_READ_SECONDS = 10.0
    ...
    def record(self, field, value, reason):
        with self.lock:
            ...
            if len(self.samples) < self.MAX_SAMPLES:
                self.samples.append(
                    {"field": str(field), "reason": reason, "value": safe_repr(value)}
                )
...
def redact(value, limit=4000):
    ...
    s = re.sub(
        r"(?i)\b(sk-[a-z0-9_-]{10,}|gh[pousr]_[a-z0-9_]{15,}|github_pat_[a-z0-9_]{15,})",
        "[REDACTED]",
        s,
    )
    s = re.sub(
...
def validate_config(raw):
    ...
    for key in (
        ...
        if (
            ...
            or any(not isinstance(x, str) or len(x) > 4096 for x in result[key])
        ):
            raise ValueError(key + " must be a list of up to 100 strings.")
    for key, lo, hi in [
        ("poll_seconds", 3, 120),
        ("history_limit", 50, 10000),
        ("codex_file_limit", 10, 3000),
        ("history_days", 1, 730),
        ("stall_seconds", 30, 86400),
        ("token_budget", 0, 10**12),
    ]:
        value = result[key]
        if (
    ...
    for url in result["opencode_urls"]:
        ...
        if (
            ...
            raise ValueError(
                "OpenCode URLs must be explicit loopback HTTP origins. Use an SSH tunnel for remote OpenCode."
            )
    ...
    if origin:
        ...
        if (
            ...
            raise ValueError(
                "public_origin must be an HTTPS origin without path, credentials or query."
            )
    ...
    if any(
        ...
        for v in result["allowed_paths"].values()
    ):
    ...
    for model, pricing in result["pricing"].items():
        if not isinstance(pricing, dict) or any(
            k not in {"input", "output", "cache_read", "cache_write"} for k in pricing
        ):
            raise ValueError(
                "Pricing fields: input, output, cache_read, cache_write; USD per million tokens."
            )
        if any(
...
def oc_usage(t, diagnostics=None):
    ...
    c = obj(t.get("cache"))
    out = dict(
        input=number(t.get("input"), diagnostics=diagnostics, field="opencode.input"),
    ...
    out["total"] = (
        number(t.get("total"), diagnostics=diagnostics, field="opencode.total")
...
def codex_usage(t, diagnostics=None):
    ...
    cached = min(
        inp,
        number(
            t.get("cached_input_tokens", t.get("cache_read_input_tokens")),
            diagnostics=diagnostics,
            field="codex.cache_read",
        ),
    )
    reasoning = number(
        t.get("reasoning_output_tokens"), diagnostics=diagnostics, field="codex.reasoning"
    )
    ...
    return {
        ...
        "cache_read": cached,
        "cache_write": 0,
        "total": number(t.get("total_tokens"), diagnostics=diagnostics, field="codex.total")
...
def make_session(source, sid, title="", directory=""):
    return {
        ...
        "source": source,
        "sources": [source],
        "title": redact(title or sid, 240),
        "directory": str(directory or ""),
        "project": path_name(directory),
        "parent_id": "",
        "relationship": "root",
        "agent": "unknown",
        "model": "unknown",
        "provider": "unknown",
        "origin": "unknown",
        "origin_evidence": "not recorded",
        "state": "unknown",
        "state_evidence": "no live connection",
        "confidence": "unknown",
        "created": 0,
        "updated": 0,
        "observed": 0,
...
def add_event(s, kind, text, ts, eid="", detail=None):
    s["events"].append(
        {
            "id": digest(s["id"], eid or ts, kind, text),
            "session_id": s["id"],
            "ts": ts or s["updated"],
            "source": s["source"],
            "kind": kind,
...
def record_usage(s, usage, ts, model=None, provider=None, cost=None):
    ...
    s["usage_known"] = True
    key = prov + "/" + name
    m = s["model_usage"].setdefault(
        key, {"model": name, "provider": prov, "usage": zero_usage(), "cost": None, "requests": 0}
    )
...
def tool_detail(name, callid, state, ts, inputs=None, output="", exit_code=None):
    ...
    return {
        ...
        "state": str(state),
        "ts": ts,
        "command": redact(command, 1500),
        "input": redact(json.dumps(inputs, ensure_ascii=False), 2000),
        "output": redact(output, 2500),
        "exit_code": exit_code,
        "files": extract_files(name, inputs),
    }
